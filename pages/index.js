    ...
    const fetchData = async () => {
      try {
        const result = await axios(
          `https://100090.pythonanywhere.com/training_test/?format=json&limit=1&offset=${currentPage}&type=get_data`
        );
        if (
          result.data.success &&
  ...
  return (
    ...
    >
      ...
      <div className="z-10 max-w-5xl w-full items-center justify-between font-mono text-sm lg:flex">
        ...
        {isLastStudent ? (
          ...
          studentData && (
            <div className="student-info">
              ...
              <p>
                Address:{" "}
                {`${studentData.address.street}, ${studentData.address.city}, ${studentData.address.state}, ${studentData.address.zip_code}`}
              </p>
        ...
        {currentPage === 0 && <p>This is the first student.</p>}
      </div>
    </main>
  );
}
